...
- (void)initTheInterface
{
    ...
    self.mainPopoverListView = [[UITableView alloc] initWithFrame:tableFrame style:UITableViewStylePlain];
    self.mainPopoverListView.backgroundColor = [UIColor whiteColor];
...
- (void)refreshTheUserInterface
{
    ...
    if (nil == self.cancelButton && nil == self.doneButton)
    {
        if (nil == _controlForDismiss)
        {
            ...
            _controlForDismiss.backgroundColor = [UIColor colorWithRed:.0 green:.0 blue:.0 alpha:.6];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    if (self.datasource && [self.datasource respondsToSelector:@selector(popoverListView:cellForRowAtIndexPath:)]) {
        UITableViewCell *cell = [self.datasource popoverListView:self cellForRowAtIndexPath:indexPath];
